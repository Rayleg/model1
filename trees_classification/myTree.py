...
class MyClassifier(BaseEstimator):
    ...
    def pruning(self, x, y, alpha):
        ...
        while r_new - r < 0.01 and len(self.tree.storage) > 3:
            r = r_new
...
def compute_impurity(ans_vector):
    ...
    freq = numpy.array([])
    for uq in numpy.unique(ans_vector):
        s = numpy.sum(ans_vector == uq)
        freq = numpy.append(freq, s)
    freq *= (1.0/len(ans_vector))
...
class TreeNode:
    ...
    def set_type_leaf(self):
        self.type = "leaf"
        self.answer = numpy.sum(self.data_y == True)*2 >= len(self.data_y)
...
class BinaryTree:
    ...
    def name_of_the_node(self, index, feature_names):
        ...
        if curr_leaf.type == "leaf":
            return str(index) + ". CLASS = " + str(curr_leaf.answer) + \
                ", div = " + str(numpy.sum(curr_leaf.data_y == False)) + "/" + str(numpy.sum(curr_leaf.data_y == True))
